...
class ServoTable:
    def __init__(self, channel, kit, kit_index=0, center_angle=90, radius=70, speed_ms=10, debug=False):
        self.channel = channel
        self.kit = kit
    ...
    def _execute_move(self, target_angle, force_smooth=True):
        ...
        min_steps = 5 if force_smooth else 1
        steps = max(min_steps, round(total_time_ms / STEP_INTERVAL_MS))
        step_delay = STEP_INTERVAL_MS / 1000

        if self.debug:
            print(f"DEBUG - angle_diff={angle_diff:.2f}, total_time_ms={total_time_ms:.0f}, steps={steps}, delay={step_delay:.3f}")
...
class MainController(MovementGenerator):
    def __init__(self, num_channels=16, simulation=True, debug=False):
        self.debug = debug
        if simulation:
    ...
    def play_frame_path(self, frame_path):
        ...
        for frame_idx, frame in enumerate(frame_path, 1):
            ...
            for name, table in self.tables.items():
                if name in frame:
                    target_angle = frame[name]
                    if self.debug:
                        print(f"[DEBUG] Moving {name} to {target_angle}")
                    table.move_to(target_angle)
